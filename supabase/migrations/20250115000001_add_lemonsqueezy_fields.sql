...
RETURNS VOID AS $$
BEGIN
  -- For Lifetime plans, ensure special handling
  IF p_subscription_type = 'lifetime' AND p_payment_status = 'paid' THEN
    ...
    SET 
      ...
      lemonsqueezy_order_id = p_order_id,
      subscription_start_date = COALESCE(p_start_date, NOW()),
    ...
    WHERE email = p_email;
  ELSE
    -- Regular subscription update logic
    UPDATE user_installations
    SET 
      lemonsqueezy_customer_id = p_customer_id,
      lemonsqueezy_variant_id = p_variant_id,
