...
import { 
  ...
  Clock,
  CheckCircle
, ArrowLeft, Bot } from 'lucide-react';
  ...
  const renderReportData = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Report Header - Enhanced for Top Gainers */}
        ...
        {isEnhancedReport() ? (
          <div className="space-y-6">
            ...
            {/* Weekly Performance Trends - Render actual charts for enhanced top_gainers */}
            {currentReport.reportType === 'top_gainers' && currentReport.data?.type === 'top_gainers_enhanced' && currentReport.data?.pages && (
              <Card className="bg-gray-800 border-gray-700">
                ...
                <CardContent>
                  <div className="space-y-6">
                    {currentReport.data.pages.slice(0, 5).map((page: any, index: number) => (
                      page.weeklyTrendData && page.weeklyTrendData.length > 0 ? (
                        <div key={page.url || index}>
                          <h4 className="text-sm font-medium text-white mb-3">
                            {index + 1}. {page.pageType || 'Other'} (+{page.clicksGained || 0} clicks)
                          </h4>
                          <div className="h-64">
                    ...
                    {(!currentReport.data.pages.slice(0, 5).some((page: any) => page.weeklyTrendData && page.weeklyTrendData.length > 0)) && (
                      <div className="text-center py-8">
                        <BarChart3 className="h-12 w-12 text-gray-600 mx-auto mb-4" />
                        <p className="text-gray-400">No weekly trend data available for this report</p>
                      </div>
                    )}
                  </div>
                </CardContent>
            ...
            {/* AI Inferred Success Patterns - Render actual saved patterns and next steps */}
            {currentReport.reportType === 'top_gainers' && currentReport.data?.type === 'top_gainers_enhanced' && (
              <Card className="bg-gray-800 border-gray-700">
                <CardHeader>
                  <CardTitle className="text-white flex items-center gap-2">
                ...
                <CardContent>
                  {currentReport.data?.aiPatterns && (currentReport.data.aiPatterns.patterns?.length > 0 || currentReport.data.aiPatterns.nextSteps?.length > 0) ? (
                    <div className="space-y-6">
                      {currentReport.data.aiPatterns.patterns?.length > 0 && (
                        <div>
                          ...
                          <ul className="space-y-2">
                            {currentReport.data.aiPatterns.patterns.map((pattern: string, index: number) => (
                      ...
                      {currentReport.data.aiPatterns.patterns?.length > 0 && currentReport.data.aiPatterns.nextSteps?.length > 0 && (
                        <Separator className="bg-gray-600" />
                      )}
                      
                      {currentReport.data.aiPatterns.nextSteps?.length > 0 && (
                        <div>
                          <h4 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
                          ...
                          <ul className="space-y-2">
                            {currentReport.data.aiPatterns.nextSteps.map((step: string, index: number) => (
                              <li key={index} className="flex items-start gap-2 text-gray-300">
                    ...
                    /* Fallback: show basic AI summary if structured patterns aren't available */
                    <div className="space-y-6">
                      <div>
                        <h4 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
                          <Lightbulb className="h-4 w-4 text-yellow-400" />
